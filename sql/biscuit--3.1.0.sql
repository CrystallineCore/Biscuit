...
-- - Regular expression support (~ !~ ~* !~*) for the subset of regexes that
--   decompose exactly to a LIKE glob -- see the REGULAR EXPRESSIONS section
--   below for what that subset is and what happens outside it
...
-- ==================== REGULAR EXPRESSIONS ====================
--
-- The ~ / !~ / ~* / !~* operators are registered as strategies 5-8, but
-- Biscuit has no regex engine. A regex qual is instead REWRITTEN into an
-- equivalent LIKE glob at plan time (biscuit_regex_to_glob(),
-- biscuit_regex.c) and then evaluated by the ordinary LIKE machinery.
--
-- The rewrite is required to be EXACT -- the glob must match precisely the
-- same strings as the regex. Biscuit is an exact-match access method: it
-- reports xs_recheck = false and nothing downstream re-tests the predicate,
-- so an approximate rewrite would silently return wrong rows rather than
-- merely slow ones.
--
-- WHAT REWRITES (and is therefore index-accelerated):
--
--   ^ and $ anchors        -- ~ is unanchored while LIKE is whole-string,
--                             so a missing ^ becomes a leading % and a
--                             missing $ a trailing %
--   literal characters     -- including \X escapes for non-alphanumeric X
--   .                      -- becomes _
--   .*  .+                 -- become % and _%
--   .{n}  .{n,}  X{n}      -- fixed and open-ended repetition counts
--
--   ^abc$        ->  abc            (exact match)
--   ^abc         ->  abc%           (prefix scan)
--   abc$         ->  %abc           (suffix scan)
--   abc          ->  %abc%          (infix scan; ~ is unanchored)
--   ^usr_[0-9]   ->  (no rewrite -- bracket expression)
--   ^a.{3}z$     ->  a___z
--
-- WHAT DOES NOT (alternation, bracket expressions, groups, unbounded or
-- optional repetition of a specific character, bounded {n,m} ranges, \d /
-- \w / \b class shorthands, backreferences, embedded-option directives):
-- these quals remain CORRECT but are not accelerated. The key is skipped,
-- the scan reports recheck so the executor re-evaluates the original regex,
-- and biscuit_costestimate() prices the path out so the planner chooses a
-- sequential scan instead. In other words an unsupported regex costs you
-- nothing but the missed optimisation.
--
-- Case sensitivity follows the opclass exactly as it does for LIKE/ILIKE:
-- ~ and !~ need the case-sensitive structures, ~* and !~* need the
-- case-insensitive (_lower) ones, so biscuit_like_ops registers only 5/6
-- and biscuit_ilike_ops only 7/8.
...
DEFAULT FOR TYPE text USING biscuit FAMILY biscuit_ops AS
    ...
    OPERATOR 5 ~ (text, text),       -- regex match
    OPERATOR 6 !~ (text, text),      -- regex non-match
    OPERATOR 7 ~* (text, text),      -- regex match (case-insensitive)
    OPERATOR 8 !~* (text, text),     -- regex non-match (case-insensitive)
    FUNCTION 1 biscuit_like_support(internal);

COMMENT ON OPERATOR CLASS biscuit_ops USING biscuit IS
'Default operator class for text types - supports LIKE, NOT LIKE, ILIKE,
NOT ILIKE, and the four regex operators (~ !~ ~* !~*) for regexes that
decompose exactly to a glob (see the REGULAR EXPRESSIONS section of
biscuit.sql). Builds both the case-sensitive and case-insensitive (_lower)
...
'Operator class for text types - supports LIKE, NOT LIKE, and the
case-sensitive regex operators ~ and !~ (for glob-decomposable regexes
only). ILIKE, NOT ILIKE, ~* and !~*
queries cannot use an index built with this opclass; the planner
...
'Operator class for text types - supports ILIKE, NOT ILIKE, and the
case-insensitive regex operators ~* and !~* (for glob-decomposable regexes
only). LIKE, NOT LIKE, ~ and !~
queries cannot use an index built with this opclass; the planner
will fall back to a sequential scan (or another eligible index) for those
